...
- Follow the repo's ESLint and Prettier standards at all times: single quotes, no semicolons, formatting via Prettier, and lint rules defined in `eslint.config.mjs`.
...
- Validate behavior with the smallest relevant command before concluding, including `pnpm lint` and `pnpm format:check` when appropriate.
